...
impl Output {
    ...
    pub fn frame(&mut self, energy: f64, pitch: f64, ks: &[f64]) {
        // `NaN.min(5514.0)` is 5514, so a NaN energy would become a full-volume frame.
        let energy = if energy.is_finite() { energy.round().min(5514.0) } else { 0.0 };
        let energy_code = quantize(&ENERGY, energy as u16);
        self.pack(energy_code, 4);
        if energy_code > 0 {
            self.pack(0, 1); // repeat
            let period_code = quantize(&PERIOD, pitch.round() as u8);
            self.pack(period_code, 6);
            ...
            // Mirror the decoder, which reads K5-K10 only for a nonzero period code.
            if period_code != 0 {
